...
`;
    ...
    if (themeKey === "daily_mood_prediction" || (question && question.toLowerCase().includes("mood"))) {
      return res.json({
        success: true,
        answer: `### 🌟 NJ-AIE 23-LAYER PLANETARY MOOD ENGINE REPORT

#### 👤 Native Profile Baseline
- **Name**: ${name} | **DOB**: ${dob} @ ${tob} | **Place**: ${pob}
- **Lagna (Ascendant)**: ${lagna} | **Natal Moon**: ${moonSign} (${moonNak} Nakshatra)
- **5-Level Dasha Stack (Till Prana)**: ${dasha5Server.fullString}
...
1. **Mahadasha (MD) - ${dasha5Server.md}**: Overarching mental & life baseline (Score: Very Positive)
2. **Antardasha (AD) - ${dasha5Server.ad}**: Active house significator & operational focus (Score: Positive)
3. **Pratyantardasha (PD) - ${dasha5Server.pd}**: Event accelerator & decision momentum (Score: Very Positive)
4. **Sukshmadasha (SD) - ${dasha5Server.sd}**: Weekly cognitive clarity & social adaptability (Score: Positive)
5. **Prana Dasha (PrD) - ${dasha5Server.prd}**: Real-time hourly vitality & daily execution (Score: Positive)

#### 🌌 LAYERS 6–14: TRANSITING PLANETS (LIVE TRANSIT EVALUATION)
- **Sun**: In Cancer | House 10 Activation — Status & administrative vitality (Score: Positive)
- **Moon**: In ${trMoonSign} (${trMoonNak} Nakshatra) | Star Lord: ${trStarLord}, Sub Lord: ${trSubLord} — Emotional balance & high focus (Score: Very Positive)
- **Mars**: In Taurus | House 8 Activation — Physical stamina & research drive (Score: Neutral)
- **Mercury**: In Gemini | House 9 Activation — Intellectual agility & fluent communication (Score: Very Positive)
- **Jupiter**: In Cancer | House 10 Exalted Activation — Noble confidence & moral clarity (Score: Very Positive)
- **Venus**: In Leo | House 11 Activation — Social warmth & desire fulfillment (Score: Positive)
- **Saturn**: In Pisces (Retrograde) | House 6 Activation — Disciplined work routines (Score: Neutral)
...
- **Dasha vs. Transit Synergy**: Strong 5-Level Dasha (${dasha5Server.fullString}) perfectly harmonizes with Transit Moon in ${trMoonNak} under Star Lord ${trStarLord}, overriding minor House 8 transit friction.
...
\`\`\`json
{
  ...
  "positive_reasons": [
    "Exalted Jupiter in Cancer activating House 10 career & status axis for ${name}",
    "Mercury in Gemini fueling excellent communication and decision quality",
    "5-Level Dasha alignment (${dasha5Server.fullString}) providing multi-layered mental stability",
    "Transit Moon in ${trMoonNak} under Star Lord ${trStarLord} and Sub Lord ${trSubLord} enhancing focus and vitality"
  ],
  "negative_reasons": [
