...
export function calculateAstrology(
  ...
  // Lahiri Ayanamsa (23.857092 in J2000.0 + precession)
  const ayanamsa = 23.857092 + 1.396042 * T + 0.000308 * T * T;
  ...
  const C_sun = (1.914602 - 0.004817 * T) * Math.sin(toRad(M_sun)) + (0.019993 - 0.000101 * T) * Math.sin(toRad(2 * M_sun)) + 0.000289 * Math.sin(toRad(3 * M_sun));
  const sunTropical = normalizeAngle(L0 + C_sun);
  const sunSidereal = normalizeAngle(sunTropical - ayanamsa);

  // Sidereal Ascendant calculation based on local time and solar position
  const localApparentTimeDegrees = (hour * 15) + longitude;
  const rawLagna = normalizeAngle(sunTropical + localApparentTimeDegrees - ayanamsa + 90);
  const lagnaSignIndex = Math.floor(rawLagna / 30);
  const lagnaDegree = rawLagna % 30;

  // 2. Moon Ephemeris (Meeus Ch 47)
  const L_prime = normalizeAngle(218.3164477 + 481267.88123421 * T);
  const M_prime = normalizeAngle(134.9633964 + 477198.8675055 * T);
  const D_moon = normalizeAngle(297.8501921 + 445267.1114034 * T);
  const F_moon = normalizeAngle(93.2720950 + 483202.0175233 * T);

  let dL = 6.288774 * Math.sin(toRad(M_prime))
         + 1.274027 * Math.sin(toRad(2*D_moon - M_prime))
         + 0.658314 * Math.sin(toRad(2*D_moon))
         - 0.185116 * Math.sin(toRad(M_sun))
         - 0.114336 * Math.sin(toRad(2*F_moon))
         + 0.058793 * Math.sin(toRad(2*D_moon - 2*M_prime))
         + 0.057066 * Math.sin(toRad(2*D_moon - M_sun - M_prime))
         + 0.053322 * Math.sin(toRad(2*D_moon + M_prime))
         + 0.045758 * Math.sin(toRad(2*D_moon - M_sun))
         - 0.040923 * Math.sin(toRad(M_sun - M_prime))
         - 0.034720 * Math.sin(toRad(D_moon))
         - 0.030383 * Math.sin(toRad(M_sun + M_prime));

  const moonTropical = normalizeAngle(L_prime + dL);
  const moonSidereal = normalizeAngle(moonTropical - ayanamsa);

  // 3. Mercury
  const M_merc = normalizeAngle(174.7947 + 149472.6741 * T);
  const L_merc = normalizeAngle(252.2508 + 149472.6741 * T);
  const C_merc = 23.44 * Math.sin(toRad(M_merc)) + 2.98 * Math.sin(toRad(2 * M_merc));
  const mercHelio = normalizeAngle(L_merc + C_merc);
  const mercGeo = normalizeAngle(sunTropical + 0.387 * Math.sin(toRad(mercHelio - sunTropical)) * 57.2958);
  const mercSidereal = normalizeAngle(mercGeo - ayanamsa);

  // 4. Venus
  const M_ven = normalizeAngle(50.4082 + 58517.8156 * T);
  const L_ven = normalizeAngle(181.9797 + 58517.8156 * T);
  const C_ven = 0.77 * Math.sin(toRad(M_ven));
  const venHelio = normalizeAngle(L_ven + C_ven);
  const venGeo = normalizeAngle(sunTropical + 0.723 * Math.sin(toRad(venHelio - sunTropical)) * 57.2958);
  const venSidereal = normalizeAngle(venGeo - ayanamsa);

  // 5. Mars
  const M_mars = normalizeAngle(19.3870 + 19140.3026 * T);
  const L_mars = normalizeAngle(355.4533 + 19140.3026 * T);
  const C_mars = 10.69 * Math.sin(toRad(M_mars)) + 0.62 * Math.sin(toRad(2 * M_mars));
  const marsHelio = normalizeAngle(L_mars + C_mars);
  const marsGeo = normalizeAngle(marsHelio + 0.65 * Math.sin(toRad(sunTropical - marsHelio)) * 57.2958);
  const marsSidereal = normalizeAngle(marsGeo - ayanamsa);

  // 6. Jupiter
  const M_jup = normalizeAngle(19.8950 + 3034.7212 * T);
  const L_jup = normalizeAngle(34.4044 + 3034.7212 * T);
  const C_jup = 5.55 * Math.sin(toRad(M_jup)) + 0.16 * Math.sin(toRad(2 * M_jup));
  const jupHelio = normalizeAngle(L_jup + C_jup);
  const jupGeo = normalizeAngle(jupHelio + 0.19 * Math.sin(toRad(sunTropical - jupHelio)) * 57.2958);
  const jupSidereal = normalizeAngle(jupGeo - ayanamsa);

  // 7. Saturn
  const M_sat = normalizeAngle(316.9670 + 1222.1138 * T);
  const L_sat = normalizeAngle(49.9443 + 1222.1138 * T);
  const C_sat = 6.36 * Math.sin(toRad(M_sat)) + 0.22 * Math.sin(toRad(2 * M_sat));
  const satHelio = normalizeAngle(L_sat + C_sat);
  const satGeo = normalizeAngle(satHelio + 0.10 * Math.sin(toRad(sunTropical - satHelio)) * 57.2958);
  const satSidereal = normalizeAngle(satGeo - ayanamsa);

  // 8. Rahu / Ketu (Mean Node)
  const omega = normalizeAngle(125.04452 - 1934.136261 * T);
  const rahuSidereal = normalizeAngle(omega - ayanamsa);
  const ketuSidereal = normalizeAngle(rahuSidereal + 180);

  const planetLongitudes: { [key: string]: number } = {
    Sun: sunSidereal,
    Moon: moonSidereal,
    Mars: marsSidereal,
  ...
  const planets: PlanetPosition[] = planetNames.map((pName) => {
    const siderealLong = planetLongitudes[pName];
    ...
    if (pName === "Sun" && house === 10) baseStrength += 15; // Sun has Digbala in 10th
    if (pName === "Moon" && house === 4) baseStrength += 15; // Moon has Digbala in 4th
    if (pName === "Jupiter" && house === 1) baseStrength += 15; // Jupiter Digbala in 1st
    if (pName === "Mercury" && house === 1) baseStrength += 15;
    if (pName === "Mars" && house === 10) baseStrength += 15;
    if (pName === "Venus" && house === 4) baseStrength += 15;
    if (pName === "Saturn" && house === 7) baseStrength += 15;
