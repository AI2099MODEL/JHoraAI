 ...
 * for report generation. An LLM will not catch internal contradictions in
 * its input data on its own — it will just narrate whatever you give it.
 *
 * This version does NOT assume any specific table names (no "JH1", "JH8",
 * etc.) or fixed field paths. It walks the JSON tree, auto-discovers
 * structures that look like planet-placement tables, house-cusp tables,
 * birth-detail blocks, and summary/header maps based on shape and field
 * names (configurable via FIELD_ALIASES), then cross-checks them against
 * each other for internal consistency.
 *
 * USAGE:
 *   const { validateChartData } = require('./chart-data-validator');
 *   const result = validateChartData(anyChartJson);
 *   if (!result.valid) {
 *     // log result.errors, block the LLM call, alert engineering, etc.
 *   }
 *   // Pass result.cleanedData (leaked placeholder/error strings stripped)
 *   // into your prompt instead of the raw object.
 *
 * CONFIGURATION:
 *   All field-name matching is alias-based and can be overridden per call:
 *     validateChartData(data, { fieldAliases: { motion: ['motionState'] } })
 *   See FIELD_ALIASES below for defaults.
 */
...
// Default field-name aliases used for fuzzy/case-insensitive key matching
// when auto-discovering table shapes. Override via opts.fieldAliases.
...
// Placeholder / stale-loading / error text that sometimes leaks into
// exported content when a backend call fails or hasn't resolved yet.
// Extend via opts.leakPatterns.
...
function approxAyanamsaForYear(year) {
  // Rough Lahiri ayanamsa estimate (~50.3"/year drift). Good enough for a
  // sanity-check tolerance band, NOT for real chart calculation.
...
// ---------------------------------------------------------------------------
// Tree walker: auto-discover table-like structures anywhere in the JSON
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Leak scanning (fully generic — no schema assumptions)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Cross-checks over auto-discovered tables
// ---------------------------------------------------------------------------
  ...
  for (const [planet, entries] of Object.entries(byPlanet)) {
    ...
    if (distinct.length > 1) {
      ...
      errors.push({
        ...
        path: `planet:${planet}`,
        message: `${planet} has conflicting motion status across tables: ${detail}. A planet cannot have two motion states for one birth moment.`,
        severity: 'error'
      });
    }
  }
    ...
    for (const rec of table.records) {
      ...
      if (diff > toleranceDeg) {
        errors.push({
          ...
          message: `${rec.planet}: tropical longitude ${tropLon.toFixed(2)}° does not align with sidereal + ayanamsa (≈${expected.toFixed(2)}°), off by ${diff.toFixed(2)}°. This table may not be derived from the same chart's actual positions.`,
          severity: 'error'
        });
      }
    }
  ...
  for (const table of cuspTables) {
    ...
    if (allIdentical) {
      errors.push({
        ...
        path: table.path,
        message: `All ${table.values.length} house cusps in this table report the same value (${table.values[0]}°). A real unequal-house system (e.g. Placidus) should not produce identical cusp values — this looks like a single value was copied across every row.`,
        severity: 'error'
      });
    }
  }
    ...
    for (const [planet, sign] of Object.entries(summary.map)) {
      ...
      if (refSign && refSign !== sign.toLowerCase()) {
        errors.push({
          ...
          message: `Summary/header shows ${planet} in ${sign}, but the more detailed table at "${reference.path}" has ${planet} in ${refMap[planet.toLowerCase()]}. Any UI or prompt text built from this summary will state the wrong sign.`,
          severity: 'error'
        });
      }
    }
  ...
  for (const block of birthBlocks.slice(1)) {
    ...
    for (const field of ['date', 'time', 'place', 'ascendant']) {
      if (first[field] && block[field] && String(first[field]) !== String(block[field])) {
        mismatches.push(`${field}: "${first[field]}" (${first.path}) vs "${block[field]}" (${block.path})`);
      }
    }
    if (mismatches.length) {
      errors.push({
        ...
        message: `Conflicting birth-detail blocks found in the same payload: ${mismatches.join('; ')}. Every downstream calculation table needs to agree on one birth record.`,
        severity: 'error'
      });
    }
  }
...
if (require.main === module) {
  const synthetic = {
    metadata: { source: 'synthetic-self-test' },
    natalRecord: { date: '1990-05-12', time: '09:15:00', place: 'Example City', ascendant: 'Leo' },
    duplicateNatalRecord: { date: '1990-05-12', time: '09:15:00', place: 'Wrong City', ascendant: 'Leo' },
    headerWidget: { Mars: 'Aries', Venus: 'Pisces' },
    tables: {
      siderealPlanets: [
        { planet: 'Mars', sign: 'Libra', absLongitude: 190.0 },
        { planet: 'Venus', sign: 'Capricorn', absLongitude: 280.0 }
      ],
      tropicalPlanets: [
        { planet: 'Mars', sign: 'Aries', degreeInSign: 10.0, motion: 'DIRECT' },
        { planet: 'Venus', sign: 'Pisces', degreeInSign: 5.0, motion: 'RETROGRADE' }
      ],
      keySignificatorsMotion: [
        { planet: 'Mars', motion: 'DIRECT' },
        { planet: 'Venus', motion: 'DIRECT' }
      ],
      houseCusps: Array.from({ length: 12 }, (_, i) => ({ cusp: i + 1, absLongitude: 15.0 }))
    },
    staleField: 'Retrieving remote database tables...'
  };

  const result = validateChartData(synthetic);
  ...
  console.log('Auto-discovered structures:', JSON.stringify(result.discovered, null, 2));
}
