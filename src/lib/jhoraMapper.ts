...
export function mapAstrologyDataToUserProfileJSON(activeUser: any, data: any): any {
  ...
  let currentSookshma: string | null = null;
  let currentPrana: string | null = null;
        ...
        if (activeB) {
          ...
          if (Array.isArray(activeB.subPeriods) && activeB.subPeriods.length > 0) {
            ...
            if (activeA) {
              ...
              if (Array.isArray(activeA.subPeriods) && activeA.subPeriods.length > 0) {
                const activeS = activeA.subPeriods.find((s: any) => {
                  const st = new Date(s.startDate || s.start);
                  const en = new Date(s.endDate || s.end);
                  return dashaNow >= st && dashaNow <= en;
                }) || activeA.subPeriods[0];
                if (activeS) {
                  currentSookshma = activeS.lord || activeS.planet;
                  if (Array.isArray(activeS.subPeriods) && activeS.subPeriods.length > 0) {
                    const activeP = activeS.subPeriods.find((p: any) => {
                      const st = new Date(p.startDate || p.start);
                      const en = new Date(p.endDate || p.end);
                      return dashaNow >= st && dashaNow <= en;
                    }) || activeS.subPeriods[0];
                    if (activeP) {
                      currentPrana = activeP.lord || activeP.planet;
                    }
                  }
                }
              }
              ...
              if (!currentSookshma) {
                const PLANET_VIM_YEARS: { [key: string]: number } = { Ketu: 7, Venus: 20, Sun: 6, Moon: 10, Mars: 7, Rahu: 18, Jupiter: 16, Saturn: 19, Mercury: 17 };
                const VIM_ORDER = ["Ketu", "Venus", "Sun", "Moon", "Mars", "Rahu", "Jupiter", "Saturn", "Mercury"];
                const aStartMs = new Date(activeA.startDate || activeM.startDate).getTime();
                const aEndMs = new Date(activeA.endDate || activeM.endDate).getTime();
                const nowMs = dashaNow.getTime();
                const pdDur = (!isNaN(aStartMs) && !isNaN(aEndMs) && aEndMs > aStartMs) ? (aEndMs - aStartMs) : 30 * 24 * 3600 * 1000;
                const baseStart = !isNaN(aStartMs) ? aStartMs : nowMs - 15 * 24 * 3600 * 1000;
                
                const startPos = VIM_ORDER.indexOf(activeA.lord) >= 0 ? VIM_ORDER.indexOf(activeA.lord) : 0;
                let curSdStart = baseStart;
                let activeSdLord = VIM_ORDER[startPos];
                let activeSdDur = pdDur;
                let activeSdStartMs = baseStart;

                for (let i = 0; i < 9; i++) {
                  const p = VIM_ORDER[(startPos + i) % 9];
                  const sdDur = pdDur * (PLANET_VIM_YEARS[p] / 120);
                  const sdEnd = curSdStart + sdDur;
                  if (nowMs >= curSdStart && nowMs <= sdEnd) {
                    activeSdLord = p;
                    activeSdStartMs = curSdStart;
                    activeSdDur = sdDur;
                    break;
                  }
                  curSdStart = sdEnd;
                }
                currentSookshma = activeSdLord;

                const sdPos = VIM_ORDER.indexOf(activeSdLord);
                let curPrdStart = activeSdStartMs;
                let activePrdLord = activeSdLord;
                for (let i = 0; i < 9; i++) {
                  const p = VIM_ORDER[(sdPos + i) % 9];
                  const prdDur = activeSdDur * (PLANET_VIM_YEARS[p] / 120);
                  const prdEnd = curPrdStart + prdDur;
                  if (nowMs >= curPrdStart && nowMs <= prdEnd) {
                    activePrdLord = p;
                    break;
                  }
                  curPrdStart = prdEnd;
                }
                currentPrana = activePrdLord;
              }
            }
          }
        }
  ...
  const profileData = {
    ...
    KP: {
      ...
      dba: {
        mahadasha: currentMaha || data.kpDasha?.dashas?.[0]?.planet || data.dashas?.[0]?.lord || "Mercury",
        bhukti: currentBhukti || data.kpDasha?.dashas?.[0]?.nested?.[0]?.planet || data.dashas?.[0]?.subPeriods?.[0]?.lord || "Saturn",
        antara: currentAntara || data.kpDasha?.dashas?.[0]?.nested?.[0]?.nested?.[0]?.planet || "Mercury",
        sookshma: currentSookshma || "Moon",
        prana: currentPrana || "Mars",
