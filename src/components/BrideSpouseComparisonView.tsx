...
export const BrideSpouseComparisonView: React.FC<BrideSpouseComparisonViewProps> = ({
  ...
  return (
    ...
    {/* KP Event Analysis & All Astro Systems Validation Tables */}
          {(comparisonResult as any).kpEventAnalysis && (comparisonResult as any).systemValidations && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className={`p-6 rounded-2xl border ${containerStyle} space-y-4`}>
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-bold text-amber-400 font-mono uppercase tracking-wider flex items-center gap-2">
                    ...
                    {((comparisonResult as any).kpEventAnalysis).title}
                  </h4>
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-amber-500/10 text-amber-400 border border-amber-500/20">{((comparisonResult as any).kpEventAnalysis).ruleId}</span>
                </div>
                <div className="space-y-3 text-xs text-slate-300">
                  <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1">
                    <span className="text-slate-400 font-bold block">7th Cuspal Sub Lord Signification:</span>
                    <p className="text-slate-200">{((comparisonResult as any).kpEventAnalysis).cslSignification}</p>
                  </div>
                  <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1">
                    <span className="text-slate-400 font-bold block">DBA Event Trigger Validation:</span>
                    <p className="text-slate-200">{((comparisonResult as any).kpEventAnalysis).dbaValidation}</p>
                  </div>
                  <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1">
                    <span className="text-slate-400 font-bold block">Ruling Planets & Query Timing:</span>
                    <p className="text-slate-200">{((comparisonResult as any).kpEventAnalysis).rulingPlanetsStatus}</p>
                  </div>
                </div>
              </div>
              ...
              <div className={`p-6 rounded-2xl border ${containerStyle} space-y-4`}>
                ...
                <div className="space-y-2.5">
                  {((comparisonResult as any).systemValidations).map((sys: any, idx: number) => (
