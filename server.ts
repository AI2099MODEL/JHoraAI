...
function saveUserAnalysisToFolder(userName: string, analysisText: string, astrologyData: any) {
  try {
    ...
    enqueueGitCommand(`git add analysis/${normName}_analysis.md analysis/${normName}_data.json && (git diff --cached --quiet || git commit -m "feat(analysis): update stored analysis for ${userName}")`, (err, stdout, stderr) => {
      if (err) {
...
async function syncProfileToGithub(action: "add" | "delete", profileName: string, profileData?: any) {
  try {
    ...
    if (action === "add") {
      ...
      enqueueGitCommand(`git add Users/userprofile.json Users/${dynamicName} && (git diff --cached --quiet || git commit -m "feat: activate user profile ${profileName}")`, (err, stdout, stderr) => {
        if (err) {
      ...
      if (fs.existsSync(usersDir)) {
        ...
        if (deletedAny && filesToGitRm.length > 0) {
          ...
          enqueueGitCommand(`git rm ${filesStr} && (git diff --cached --quiet || git commit -m "feat: deactivate user profile ${profileName}")`, (err, stdout, stderr) => {
            if (err) {
...
app.post("/api/user-profile/index-table", async (req, res) => {
  try {
    ...
    enqueueGitCommand(`git add Users/userprofile.json Users/${dynamicName} documents/master_astro_handbook.md && (git diff --cached --quiet || git commit -m "feat: index ${tableId} for profile ${fallbackName}")`, (err, stdout, stderr) => {
      if (err) {
...
`;
    ...
    // Helper to generate dynamic event report filtered by Mood Engine activated houses
    const generateDynamicEventReport = (key: string, label: string) => {
      // Mood Engine Activated Houses
      let activeHousesList = [1, 2, 3, 6, 10, 11];
      if (key === "daily_mood_prediction" || (question && question.toLowerCase().includes("mood"))) {
        activeHousesList = [1, 3, 4, 7, 8, 12];
      }

      const housesHeader = `### 🏠 Mood Engine Activated Houses: Houses ${activeHousesList.join(", ")}
*(Identified via 7-Layer Mood Stack: MD ${dasha5Server.md}, AD ${dasha5Server.ad}, PD ${dasha5Server.pd}, SD ${dasha5Server.sd}, PrD ${dasha5Server.prd} & Active Planets [Mercury, Saturn, Jupiter])*\n`;

      const catalog: Record<string, { title: string; houses: number[]; bullets: string[] }[]> = {
        daily_mood_prediction: [
          {
            title: "Domestic Pressure & Need for Rest",
            houses: [4, 12],
            bullets: [
          ...
          {
            title: "Personal & Family Interactions",
            houses: [2, 4],
            bullets: [
          ...
          {
            title: "Partnership Friction & Unexpressed Thoughts",
            houses: [3, 7],
            bullets: [
          ...
          {
            title: "Deep Analytical Research & Self-Introspection",
            houses: [3, 8],
            bullets: [
        ...
        career_promotion: [
          {
            title: "Promotion & Executive Elevation",
            houses: [2, 6, 10, 11],
            bullets: [
          ...
          {
            title: "Competitive Performance & Operational Success",
            houses: [1, 6, 10, 11],
            bullets: [
          ...
          {
            title: "Financial Gains & Salary Appreciation",
            houses: [2, 6, 11],
            bullets: [
          ...
          {
            title: "Workplace Communication & Strategic Proposals",
            houses: [3, 10],
            bullets: [
              "Enhances negotiation quality, technical documentation, and strategic proposals to leadership.",
              "Favorable alignment for client communications and team presentations."
            ]
          }
        ],
        finance_wealth: [
          {
            title: "Financial Inflow & Liquidity Surge",
            houses: [2, 6, 11],
            bullets: [
          ...
          {
            title: "Strategic Investments & Asset Growth",
            houses: [2, 5, 11],
            bullets: [
          ...
          {
            title: "Receivables & Debt Clearance",
            houses: [6, 11],
            bullets: [
        ...
        foreign_travel_settlement: [
          {
            title: "International Journey & Overseas Relocation",
            houses: [3, 9, 12],
            bullets: [
              "Strong activation of long-distance travel, overseas assignments, and foreign stays.",
              "Foreign land houses (3, 9, 12) align to open distant opportunities."
            ]
          },
          {
            title: "Visa, Passport & Documentation Clearance",
            houses: [3, 11],
            bullets: [
              "Favorable window for processing travel visas, work permits, and official passports.",
              "Documentation significators (3 + 11) support smooth paper verification."
            ]
          },
          {
            title: "Overseas Settlement & Distant Gains",
            houses: [9, 12, 11],
            bullets: [
              "Long-term foreign residence potential, overseas connections, and international gains.",
              "Fulfillment houses (9 + 12 + 11) support foreign settlement."
            ]
          }
        ],
        marriage_first: [
          {
            title: "Partnership Union & Marital Harmony",
            houses: [2, 7, 11],
            bullets: [
              "Strong indication of bonding, family consensus, and formal relationship commitments.",
              "Union houses (2, 7, 11) align to support partnership stability."
            ]
          },
          {
            title: "Romantic Alignment & Mutual Affection",
            houses: [5, 7, 11],
            bullets: [
              "Deep emotional connection, pleasant companionship, and empathetic communication.",
              "Affection significators (5 + 7 + 11) foster mutual warmth."
            ]
          },
          {
            title: "Shared Commitment & Family Addition",
            houses: [2, 7],
            bullets: [
        ...
        health_disease: [
          {
            title: "Robust Vitality & Rapid Recovery",
            houses: [1, 5, 11],
            bullets: [
              "High physical energy reserves, strong immune defense, and rapid recovery.",
              "Constitution houses (1, 5, 11) align to boost bodily stamina."
            ]
          },
          {
            title: "Metabolic Balance & Physical Endurance",
            houses: [1, 3, 11],
            bullets: [
              "Active bodily stamina, mental endurance, and balanced daily metabolism.",
              "Vitality significators (1 + 3 + 11) neutralize physical fatigue."
            ]
          },
          {
            title: "Preventive Health & Disease Defense",
            houses: [5, 11],
            bullets: [
              "Effective defense against ailments and health friction through disciplined habits.",
              "Recovery houses (5 + 11) protect against chronic vulnerabilities."
            ]
          }
        ],
        property_vehicle: [
          {
            title: "Property & Vehicle Acquisition",
            houses: [4, 11, 2],
            bullets: [
              "High probability of acquiring real estate, homes, vehicles, or fixed assets.",
              "Asset houses (4, 11, 2) align to support property deals."
            ]
          },
          {
            title: "Asset Value Appreciation & Domestic Luxury",
            houses: [4, 11],
            bullets: [
              "Value appreciation of existing holdings, home improvement, and domestic comfort.",
              "Fulfillment significators (4 + 11) support asset security."
            ]
          },
          {
            title: "Legal Verification & Title Deed Clearance",
            houses: [2, 4, 11],
            bullets: [
              "Smooth verification of property titles, registration deeds, and asset loan approvals.",
              "Document significators (2 + 4 + 11) facilitate legal clarity."
            ]
          }
        ],
        litigation: [
          {
            title: "Victory in Disputes & Competitive Edge",
            houses: [1, 6, 11],
            bullets: [
              "Dominance over opposing claims, winning legal disputes, or securing compromises.",
              "Litigation houses (1, 6, 11) align to give strong negotiation leverage."
            ]
          },
          {
            title: "Court Authority & Legal Documentation",
            houses: [6, 10, 11],
            bullets: [
              "Strong legal documentation, favorable official verdicts, and institutional backing.",
              "Official authority significators support structured presentations."
            ]
          },
          {
            title: "Amicable Financial Settlement",
            houses: [2, 6, 11],
            bullets: [
              "Favorable monetary compensation or structural terms in dispute resolutions.",
              "Settlement houses (2 + 6 + 11) promote clear compromises."
            ]
          }
        ],
        education: [
          {
            ...
            houses: [4, 9, 11],
            bullets: [
              "Superior concept absorption, high examination performance, and admissions.",
              "Education houses (4, 9, 11) align to support academic excellence."
            ]
          },
          {
            title: "Intellectual Mastery & Research Excellence",
            houses: [5, 9, 11],
            bullets: [
              "Command over complex technical topics, analytical research, and problem-solving.",
              "Intelligence significators (5 + 9 + 11) foster deep retention."
            ]
          },
          {
            title: "Degree & Certification Success",
            houses: [4, 11],
            bullets: [
              "Successful completion of examinations, degree submissions, and certifications.",
              "Recognition houses (4 + 11) ensure academic milestone completion."
            ]
          }
        ]
      };

      const domainList = catalog[key] || catalog.career_promotion;
      
      // Filter events by active houses
      let filtered = domainList.map(ev => {
        const matchingHouses = ev.houses.filter(h => activeHousesList.includes(h));
        return { ...ev, matchingHouses, isMatch: matchingHouses.length > 0 };
      }).filter(e => e.isMatch);

      if (filtered.length === 0) {
        filtered = domainList.map(ev => ({ ...ev, matchingHouses: ev.houses, isMatch: true }));
      }

      const matchesSection = filtered.map(ev => 
        `- **${ev.title}** *(Activated Houses: ${ev.matchingHouses.join(", ")})*\n` +
        ev.bullets.map(b => `  - ${b}`).join("\n")
      ).join("\n\n");
...
### 🌗 Secondary Modifiers (${dasha5Server.prd} Prana & ${trMoonSign} Moon Sign Overlay)
...
`;
      ...
      return `${housesHeader}
### 🌟 Primary KP Eventbook Combination Matches (Filtered by Active Houses)

${matchesSection}

---

### 🌗 Secondary Modifiers (Transit Moon & House Overlay)

- **Active Dasha Alignment**: 5-Level Dasha (${dasha5Server.fullString}) triggers primary domain significators.
- **Transit Moon Layer**: Transiting Moon in ${trMoonNak} (${trMoonSign}) activates real-time daily execution.

---

### 🎛️ Domain Parameters & Evaluation

- **Domain Feasibility**: High (Rating: 4.4 / 5 Stars)
- **Execution Momentum**: Strong (Supported by active Dasha lords)
...
#### 🟢 Positive Reasons:
- Exalted Jupiter providing structural support for ${name || 'Nitin'}
- Mercury in Gemini fueling communication and decision quality
- Strong 5-Level Dasha alignment (${dasha5Server.fullString}) providing multi-layered stability
- Transit Moon in ${trMoonNak} under active Star Lord ${trStarLord} and Sub Lord ${trSubLord}

#### 🔴 Negative Reasons:
- Retrograde Saturn in House 6 requires steady routine discipline to avoid friction
...
#### Best For:
- Advancing key goals aligned with activated houses ${activeHousesList.join(", ")}.
- Strategic decision-making, team collaboration, and documentation.
- Focusing energy on high-probability opportunities today.

#### Avoid:
- Impulsive actions in inactive house domains without proper verification.
- Overcommitting beyond operational capacity.
- Unnecessary confrontation or arguments.
`;
    };

    if (themeKey || (question && (
      question.toLowerCase().includes("mood") ||
      question.toLowerCase().includes("career") ||
      question.toLowerCase().includes("finance") ||
      question.toLowerCase().includes("travel") ||
      question.toLowerCase().includes("marriage") ||
      ...
      const activeKey = themeKey || (
        question.toLowerCase().includes("mood") ? "daily_mood_prediction" :
        question.toLowerCase().includes("career") ? "career_promotion" :
        question.toLowerCase().includes("finance") ? "finance_wealth" :
        question.toLowerCase().includes("travel") ? "foreign_travel_settlement" :
        question.toLowerCase().includes("marriage") ? "marriage_first" :
        question.toLowerCase().includes("health") ? "health_disease" :
        question.toLowerCase().includes("property") ? "property_vehicle" :
        question.toLowerCase().includes("litigation") ? "litigation" :
        question.toLowerCase().includes("education") ? "education" : "career_promotion"
      );

      return res.json({
        success: true,
        answer: generateDynamicEventReport(activeKey, themeLabel || "Activated Events")
      });
...
function runAnalysisSyncAgentForProfile(profile: any, filename?: string) {
  try {
    ...
    enqueueGitCommand(`git add analysis/${profileFolder}/static_data.json analysis/${profileFolder}/dynamic_data.json && (git diff --cached --quiet || git commit -m "feat(analysis): update static and dynamic logs for ${profileName}")`, (err, stdout, stderr) => {
      if (err) {
