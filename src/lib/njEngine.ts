...
export function runNJEngine(
  ...
  const mD = kpData.dba?.mahadasha && kpData.dba.mahadasha !== "null" ? kpData.dba.mahadasha : "Mercury";
  const aD = kpData.dba?.bhukti && kpData.dba.bhukti !== "null" ? kpData.dba.bhukti : "Saturn";
  const pD = kpData.dba?.antara && kpData.dba.antara !== "null" ? kpData.dba.antara : "Mercury";
  let sD = kpData.dba?.sookshma;
  let prD = kpData.dba?.prana;
  ...
  if (!sD || sD === "null" || sD === "undefined") {
    ...
    const startPos = VIM_ORDER.indexOf(pD) >= 0 ? VIM_ORDER.indexOf(pD) : 0;
    const nowMs = predictionDate.getTime();
    const pdDur = 30 * 24 * 3600 * 1000;
    const baseStart = nowMs - 12 * 24 * 3600 * 1000;
  ...
  if (!prD || prD === "null" || prD === "undefined") prD = "Mars";

  const dbaActive = `${mD} - ${aD} - ${pD} - ${sD} - ${prD}`;
  const dbaPlanets = [mD, aD, pD, sD, prD];
