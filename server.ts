...
function saveUserAnalysisToFolder(userName: string, analysisText: string, astrologyData: any) {
  try {
    ...
    exec(`git add analysis/${normName}_analysis.md analysis/${normName}_data.json && (git diff-index --quiet --cached HEAD || git -c user.name="AI Studio Agent" -c user.email="agent@aistudio.google" commit -m "feat(analysis): update stored analysis for ${userName}")`, (err, stdout, stderr) => {
      if (err) {
...
async function syncProfileToGithub(action: "add" | "delete", profileName: string, profileData?: any) {
  try {
    ...
    if (action === "add") {
      ...
      exec(`git add Users/userprofile.json Users/${dynamicName} && (git diff-index --quiet --cached HEAD || git -c user.name="AI Studio Agent" -c user.email="agent@aistudio.google" commit -m "feat: activate user profile ${profileName}")`, (err, stdout, stderr) => {
        if (err) {
      ...
      if (fs.existsSync(usersDir)) {
        ...
        if (deletedAny && filesToGitRm.length > 0) {
          ...
          exec(`git rm ${filesStr} && (git diff-index --quiet --cached HEAD || git -c user.name="AI Studio Agent" -c user.email="agent@aistudio.google" commit -m "feat: deactivate user profile ${profileName}")`, (err, stdout, stderr) => {
            if (err) {
...
app.post("/api/user-profile/index-table", async (req, res) => {
  try {
    ...
    exec(`git add Users/userprofile.json Users/${dynamicName} documents/master_astro_handbook.md && (git diff-index --quiet --cached HEAD || git -c user.name="AI Studio Agent" -c user.email="agent@aistudio.google" commit -m "feat: index ${tableId} for profile ${fallbackName}")`, (err, stdout, stderr) => {
      if (err) {
...
function runAnalysisSyncAgentForProfile(profile: any, filename?: string) {
  try {
    ...
    exec(`git add analysis/${profileFolder}/static_data.json analysis/${profileFolder}/dynamic_data.json && (git diff-index --quiet --cached HEAD || git -c user.name="AI Studio Agent" -c user.email="agent@aistudio.google" commit -m "feat(analysis): update static and dynamic logs for ${profileName}")`, (err) => {
      if (err) {
