  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setProfile(null);
        setErrorMsg("No active profile found in Profile Service. Please cast a horoscope or configure your profile.");
      }
    } catch (err: any) {
      console.warn("Failed to load user profile from backend:", err);
      setProfile(null);
      setErrorMsg("Failed to connect to profile service. Please check your connection or cast a new horoscope.");
    } finally {
                      ...
                      <div className="space-y-3 pt-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {primaryThemeHouses.slice(0, 4).map((h) => {
                            ...
                            if (!m) return null;
                            return (
                              <div key={h} className="bg-slate-950/80 border border-slate-800 p-3 rounded-lg flex items-start gap-2.5">
                                <span className="font-mono text-xs font-bold text-indigo-300 bg-indigo-500/20 w-5 h-5 flex items-center justify-center rounded border border-indigo-500/30 shrink-0">
                                  {h}
                                </span>
                                <div>
                                  <strong className="block text-xs font-bold text-white">{m.title}</strong>
                                  <span className="text-[11px] text-slate-200 leading-normal block mt-0.5 font-sans font-normal">
                                    {m.desc}
                                  </span>
                                </div>
                              </div>
                    ...
                    <table className="w-full text-left text-xs text-slate-800 font-mono">
                      ...
                      <tbody className="divide-y divide-slate-800/40">
                        {Array.from({ length: 12 }).map((_, idx) => {
                          ...
                          return (
                            <tr key={house} className="hover:bg-slate-900/40 transition-colors">
                              <td className="py-2.5 font-bold text-indigo-400">JH{house}</td>
                              <td className="py-2.5 font-sans">
                                <div className="font-semibold text-slate-100 text-[11px]">{m?.title || `House ${house}`}</div>
                                <div className="text-[10px] text-slate-300 line-clamp-1">{m?.desc || "No custom significations."}</div>
                              </td>
                              <td className="py-2.5 text-center text-slate-100 font-bold">{count} / 8</td>
