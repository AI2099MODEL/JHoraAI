  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const apiResponse = await fetch("/api/astrology/ai-relationship-expert", {
        ...
        body: JSON.stringify({
          evidence,
          question: `Generate a high-fidelity ${selectedReportType} for ${astrologyData.birthDetails?.name || (astrologyData as any).nativeName || "Native"} and ${activePartner} at age ${targetAge}. Please organize the response with clear headers, citation references, and bespoke recommendations tailored to this query.`
        })
      });
      ...
      const profileName = astrologyData.birthDetails?.name || (astrologyData as any).nativeName || "Vedic Native";
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <div className={`px-3 py-2 rounded-xl text-xs font-semibold ${isDark ? "bg-slate-900 text-slate-300" : "bg-slate-200 text-slate-700"}`}>
                  {astrologyData.birthDetails?.name || (astrologyData as any).nativeName || "Vedic Native"}
                </div>
              </div>
